...
36. **用量账不做钱。** Pico 只记 `usage_events`（谁/校/kind/后端模型/token 或诚实 unknown）。积分门脸 = 同一本账派生（服务端换算，三位小数）；edu 导出用 Pico 给出的积分数字，禁止再乘。禁在 Pico 建点池/余额。char/4 估数不能给 edu 计费，也不得留在账上（启动 scrub 成 unknown）。档位不得当 `model`。钱/钱包在 edu-core，拉 `GET /v1/internal/usage/export`（`PICO_HOOK_SERVICE_TOKEN`）。
