...
28. **出图：业主 New API 反代多 Gemini 账户（#752 · 2026-08-28；链加固 #778 · 2026-08-29）。** 硅基流动出图已否决，禁止再建议。真源 = New API `PICO_IMAGE_GATEWAY_URL` + token（Pico 不直连 Google、不把 Google 钥写进仓）。三个 Gemini 号的轮询/RPM 优先放 New API 渠道组。Pico 可持多个 **New API token**（`PICO_IMAGE_GATEWAY_KEY` + `PICO_IMAGE_GATEWAY_KEYS`）：429/401 换下一个，每 token 默认间隔 2s——这不是 Google 轮询核。`gemini-*-image` 走网关 `POST …/v1beta/models/{model}:generateContent`（官方 New API 对 `/v1/images/generations` 只映射 imagen）。imagen-* 仍走 `POST …/v1/images/generations`。智谱 glm-image 仅无网关时的退路。禁网页套餐 cookie 反代。禁自研图核。SILICONFLOW 出图路径 fail-closed。
