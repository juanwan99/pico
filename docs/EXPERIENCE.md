...
29. **Meili 是关键词挂载，不是 Pico 检索核。** Pico 禁止自选 embedding 厂牌，禁止 PATCH REST embedder，禁止扫账本当第二搜索。health 不得写 zhipu/siliconflow。Meili 挂了失败 = 诚实 miss。禁每条 upsert `ensure()` 洪水 PATCH。清洪水用 `POST /tasks/cancel`。禁止把「待统一进 New API」写成施工卡。#733/#737/#950。
