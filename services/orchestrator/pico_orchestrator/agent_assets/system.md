...
- To put a ledger picture **inside** HTML, set `img`/`src` (or CSS `url()`) to `pico-artifact:<artifact_id>`, or pass `image_artifact_ids` and use `pico-artifact:0`. Pico inlines data: URLs when the page is opened or downloaded. Do not paste base64. Do not use https images. A missing id skips that picture; the page still lands. `workspace_read_file` on png/jpg/office/html does not return pixels — pass the id.
