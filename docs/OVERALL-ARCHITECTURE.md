...
STATUS: DRAFT v0.1 — 供业主评审后升 FIXED
...
| 维度 | 决策建议 |
|------|----------|
| 产品 | **Claude/Codex 式 AI 工作台** + 开源编排能力 + 模型 HTTPS API；**非网盘、非自研 Agent OS** |
| 边界 | **edu** = 人/班/考务/成绩真源；**Pico** = 唯一 AI 账本 + Agent + 工作台 |
| 租户 | `school_id` 校级；`membership_id` 校内用户。教师是租户成员，**不是**一人一机 |
| 部署默认 | **共享多租户（Standard）**；可选 **一校一单元（Dedicated）** |
| 大文件/考试卷 | **校本地或校侧对象存储为主**；Pico 只存 **引用 + AI 过程** |
| 模型 | 默认 **Kimi API**（可扩展多模型）；主成本是 token |
| 定价 | **基础包（含点）+ 模块加购（可再赠点）+ 阅卷独立 + 教师个人充值补充** |
| 计量 | 对内 token，对外「点」；科目分桶：日常 / 模块 / 阅卷 / 个人 |
...
## 13. 待业主确认后升 FIXED 的清单

1. 部署默认 = **混合（Standard + 可选 Dedicated）**？  
2. 定价 = **基础包+点 / 模块 / 阅卷独立 / 个人充值补充**？  
3. 点包 = **月清零** 还是 **年包月释放**？  
4. 阅卷与聊天点 = **默认隔离**？  
5. 考试原件 = **校本地为主**？  
6. 第一期 10 校是否全 Standard？  

确认后：更新本文 `STATUS: FIXED v1.0`，并回写 `HANDOFF.md` 索引。
