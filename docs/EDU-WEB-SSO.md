...
| Materials | Workbench lists/searches via `/v1/edu/materials` as this membership. Unchecked → no school file bodies in the round. |
| Fail | Invalid/spent ticket → Pico `/login`. Do not take down edu |

Forbidden: iframe, edu `/pico` subpage, parent-domain `.weiyuji.cn` cookies, field/student/page/material **bodies** on the ticket or jump URL, school-wide service dump, hot-patching a running container. Named item **ids** may ride the one-time ticket.
