...
28. **出图：Gemini 官方或业主 New API 网关（#752 · 业主 2026-08-28）。** 硅基流动出图已否决，禁止再建议。优先 `GEMINI_API_KEY` → `POST generativelanguage.googleapis.com/…/gemini-2.5-flash-image:generateContent`，或 `PICO_IMAGE_GATEWAY_URL` + `PICO_IMAGE_GATEWAY_KEY`（OpenAI 兼容 `/v1/images/generations`）。智谱 glm-image 仅无上述钥时的退路。禁网页套餐 cookie 反代。禁 Pico 内多账户轮询。禁自研图核。SILICONFLOW 出图路径 fail-closed。
