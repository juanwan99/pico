...
37. **聊天回形针 = 老师唯一上传口。** 图和文档同一回形针：图本轮进 Pi `images[]`；文档落 `POST /v1/files`，抽出失败也落盘。数字 PDF/Office **有正文则注入本轮 user**（文件名+摘录，不靠 pico-fast 自己想起调工具），仍可用 `workspace_read_file` 按文件名读。扫描 PDF：Pi 0.84.4 `prompt()` 无 DocumentContent，禁止把 PDF 当 ImageContent 硬塞；用已有 pypdfium2 渲页进现成 `images[]`（上限 32）。**禁止把「没抽出正文 / 没有文字层 / 请再截图 / 读不了」焊进 user。** `/c/new` 未绑会话：只认领最近 3 分钟内、距最新一份 ≤90 秒的未绑突发，禁止把更早的残留卷进本轮。像素不进 prompt 正文。LibreChat 若带了 `file`/`input_file` 段，文件名要进本轮（禁止静默丢掉后让模型说没收到）。LibreChat 本地仓不是老师柜。ingest HTTP 失败不得假装已传。「我的文件」是生成物柜，不是第二本地上传口。禁目的地三选 / VectorStore / 转到学校当上传。禁第二套文件 OS。旧 `.doc/.ppt/.xls` 进账本后 **工具观察**人话另存，不焊进老师原话，不自研转换核。禁自研 PDF 核。
...
41. **二进制不进脑（#850）。** 像素只走账本；脑只说 id。HTML 嵌图 = `src="pico-artifact:<id>"`（或 `image_artifact_ids` + `pico-artifact:0`）；账本存 id，打开/下载时内联 data:。GET 任务列表不夹带大段 utf8（结果区 2s 轮询）。缺 id 跳过该图 + observation 诚实，不毁整页。作者 `body` 仍 20 万。`workspace_read_file` 对 png/jpg 不回像素；对 pdf/docx/xlsx/pptx 回抽出正文（不是 base64）；HTML 剥 data:。edu 侧栏要读文件（#857）：天花板含 `workspace_read_file`，仍禁 generate/edit/出图/落盘。PDF 抽正文先走已有 pypdfium2 文本层，空页再 RapidOCR；现网缺 OCR onnx 时数字 PDF 仍可读。无文字层则同渲染器出页图进现成 `images[]`（上限 32），**禁止把「没抽出正文 / 读不了」写进 user 或工具观察当否决。** 禁自研 PDF 核。流死（`stream_read_error` / eof / soft_errors）人话失败，已落盘成品仍可见。追问「做完了吗」复述已有文件名不是假绿：结果区按对话柜留文件，不因最新一轮 0 产物清空。禁焊课件管道、禁 CDN、禁把模型当 U 盘。聊天口默认拒绝过长 prompt 的上限是 10 万字（拒绝，不静默截断），不是 1.2 万。
