...
48. **办公脚本在 `pico-office` 容器跑，不在 pico-api 里（#959）。** `sandbox_office_lib` 合同不变，后端 = 同 `pico-sandbox` 镜像的第二个 service：`network_mode: none`、`read_only`、tmpfs、uid 65532、unix socket `/run/pico-office/office.sock`，pico-api 用 `PICO_OFFICE_URL=unix://…` 进去；测试用 `embedded`（conftest），生产禁 `embedded`。箱内是完整 Python（stdlib / csv / pandas / Pillow / matplotlib / soffice），**Pico 不再维护 import 白名单 / AST jail / pathlib 桩 / 猴补 save**（`sandbox_exec.py` 已删）；「不要 bash」= 不给宿主 shell、不开宿主 Pi builtins、不注册 `exec` 类动词。收集规则：`OUTPUT_PATH` → 脚本记录的保存路径 → 工作目录最新 `*.kind`；都没有 = `sandbox.no_output` 人话。脚本报错原样回 stderr 让模型自己修。`artifact_id` 原件放宽到任何类型（CSV/TXT/PNG…），只有同 kind 的办公名才必须是真 OOXML。`sandbox_workspace_exec`（parse-only 假执行）已从网关拆除。加 rlimit 只在容器（`PICO_OFFICE_RLIMIT_NPROC`），共享宿主上 NPROC 帽会误杀。**部署坑（#962）：** 新 service 的 socket/数据目录不要用命名卷（挂进来 root 所有，65532 写不进）；用宿主绑定并在 prod-update 里 `mkdir + chmod 1777 + chown 65532`，Docker 预建的 root 空目录要 `rmdir` 重建而不是 `chmod || true` 吞掉；bash 在 checkout 前已读入旧 `prod-update.sh`，**改了部署脚本的那一次部署跑的还是旧脚本**，新门下一次才生效；重建绑定源目录后，另一侧容器（pico-api）必须 restart 才能重新绑到新 inode。部署门必须探箱（`exit 11`），只探 pico-api health 会假绿。真模型回归：`scripts/office-regress.py`（生产 loopback · 代理钥 + `X-Pico-Membership-Id` 合成租户 · 按 `kind` 找产物，同名替换是产品规则别按文件名找），部后跑、结果贴 Issue、不进 CI。
