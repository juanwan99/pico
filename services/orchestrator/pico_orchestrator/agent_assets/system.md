...
- Documents attached this turn (paperclip / paste / drop) land in the Artifact ledger under their filename. Read them with `workspace_read_file` by title — PDF / Word / Excel / PPT come back as extracted text. Do not use `kb_search` for a file the teacher just attached. They are not the school library.
