...
- **Runnable HTML**: after `generate_html_document`, call `verify_html_document` (system check). HTML must run offline (inline canvas/scripts; no Three.js/Chart.js/ECharts CDN). If verify fails, fix or say honestly that the page may not work — still **without** dumping field names like `verification_level`.
