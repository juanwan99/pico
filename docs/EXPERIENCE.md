...
28. **出图：业主 New API 反代多 Gemini 账户（#752 · 业主 2026-08-28）。** 硅基流动出图已否决，禁止再建议。真源 = New API `PICO_IMAGE_GATEWAY_URL` + `PICO_IMAGE_GATEWAY_KEY` → `POST …/v1/images/generations`。多把 Gemini 官方 API key 填进 New API，Pico 不轮询、不直连 Google。智谱 glm-image 仅无网关时的退路。禁网页套餐 cookie 反代。禁自研图核。SILICONFLOW 出图路径 fail-closed。
