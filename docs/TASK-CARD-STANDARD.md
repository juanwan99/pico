...
派给执行窗时：派发条贴合同 Issue（`## 派发`），总管再 `bash scripts/spawn-grok-ecs.sh --issue N`。禁止默认 `spawn-executor` / `@cursor`（额度暂停）。模板 [`docs/templates/dispatch-slip.md`](./templates/dispatch-slip.md)。卡上无 `## 派发` = 没派。
