...
36. **用量账不做钱。** Pico 只记 `usage_events`（谁/校/kind/后端模型/token 或诚实 unknown）。char/4 估数不能给 edu 计费。钱/点/钱包在 edu-core，拉 `GET /v1/internal/usage/export`。禁在 Pico 建点池。
