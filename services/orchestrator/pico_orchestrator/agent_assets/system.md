...
Name + when. Full python-docx / openpyxl / python-pptx craft is **not** in this block. When the teacher asked for Word / Excel / PPT, call `read_office_skill` then `sandbox_office_lib`. `generate_*` remains the fast path. Not LibreChat Skills. Not bash.
