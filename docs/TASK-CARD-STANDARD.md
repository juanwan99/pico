...
派给本窗时：派发条可贴合同 Issue（`## 派发`），执行者就是业主正在说话的这扇 Grok 沙箱。ECS 只部。禁止 spawn-executor / Cursor 云 Task（入口已断）。合前 `pr-ci-ready` 一眼，禁止窗里轮询 CI。模板 [`docs/templates/dispatch-slip.md`](./templates/dispatch-slip.md)。卡别拆太细。
