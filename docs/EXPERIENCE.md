...
37. **聊天回形针 = 老师唯一上传口。** 图和文档同一回形针：图本轮进 Pi `images[]`；文档落 `POST /v1/files`，抽出失败也落盘；抽出正文注入本轮 user prompt（不靠 pico-fast 自己想起调工具），仍可用 `workspace_read_file` 按文件名读。扫描/无文字层 PDF：已有 pypdfium2 渲页（最多 8）进现成 vision `images[]`（上传 worker ≠ 聊天 worker 时聊天时重渲）。禁止把「没抽出正文」焊成读不了或让老师再截图。像素不进 prompt 正文。LibreChat 本地仓不是老师柜。ingest HTTP 失败不得假装已传。「我的文件」是生成物柜，不是第二本地上传口。禁目的地三选 / VectorStore / 转到学校当上传。禁第二套文件 OS。旧 `.doc/.ppt/.xls` 进账本后人话另存，不自研转换核。禁自研 PDF 核。
