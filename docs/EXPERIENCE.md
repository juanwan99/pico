...
24. **办公文档 = [ADR-OFFICE-DOC-PIPELINE](./ADR-OFFICE-DOC-PIPELINE.md)。** spec/`generate_*` = **已拆污染**（#952 网关拆除），不是天花板、不是快路。卡 1 #690 / 卡 2 #694 已收口。禁再加厚 spec 当天花板。上限 = 文档 skill 工艺 + 隔离面真跑办公库。`sandbox_office_lib` = 隔离 python-docx / openpyxl / python-pptx。办公工艺全文不进 SYSTEM：目录一行，按需 `read_office_skill`。禁 host bash、禁 Pi 内建 bash、禁自研幻灯 OS。Pi 不在宿主写 python-docx；隔离办公 Python ≠ 开放编程产品。
...
28. **出图：业主 New API 反代多 Gemini 账户（#752 · 业主 2026-08-28）。** 硅基流动出图已否决，禁止再建议。真源 = New API `PICO_IMAGE_GATEWAY_URL` + `PICO_IMAGE_GATEWAY_KEY`（Pico 不轮询、不直连 Google / 智谱）。`gemini-*-image` 走网关 `POST …/v1beta/models/{model}:generateContent`（官方 New API 对 `/v1/images/generations` 只映射 imagen）。imagen-* 仍走 `POST …/v1/images/generations`。禁网页套餐 cookie 反代。禁自研图核。SILICONFLOW 出图路径 fail-closed。智谱 glm-image 直连已删。**聊天脑同样只打 New API**（`DEEPSEEK_BASE_URL=http://127.0.0.1:3000/v1` · 模型仍 `gpt-5.6-sol` · `api=openai-responses`）。AIProxy 是 New API 的 Custom 渠道上游（`…/openai/responses`，禁止带 `/v1`），不是 Pico 直连。Wei-Shaw/sub2api 只绑 host loopback，作账号登录态；轮询/计费在 New API。禁止把 Sub2API SPA 当老师前端或 cookie 接管 pico.aivia.asia。管理者页 `/admin/gateway`（ADMIN only）面向所有者：薄读 `channel-monitors` 画 7 日/168h 状态盘，软按钮只转发 refresh/test/clear-error/recover-state；硬重登/签合规走独立公网 `https://workbench.aivia.asia`（Sub2API 真页 · 密码 + 谷歌验证器）。禁止挂到 pico.aivia.asia。Pico 不代签合规，不自研账号 OS。机上 `.env` 用 `SUB2API_ADMIN_EMAIL` / `SUB2API_ADMIN_PASSWORD` / `SUB2API_ADMIN_API_KEY`（值不进 git）。业主绑 Google Authenticator 后密码登录返回 `requires_2fa`，Pico 薄读走 `X-API-Key`（不是 Bearer JWT）。Dify 运行面退役；`workbench.aivia.asia` 主机名只给所有者账号台。
29. **Meili 是检索挂载，不是 Pico 检索核。** 索引 = chunk 级（#1006）。embedding / rerank **只准经 New API**（业主 2026-09-14 · #1005）：Meili REST embedder 只指向 `{DEEPSEEK_BASE_URL}/embeddings`（默认 `embedding-3` · 2048 维 · 模板 `title` + `text|truncate:180`；#1020 把全文+标题塞进模板掉点，因 CJK 无空格不能用 truncatewords），检索后可 `POST /v1/rerank`（`PICO_KB_RERANK` 默认关；开启后分数无区分则不改序，退回 Meili）。标题检索只补 hybrid 没召回的文件、接在 hybrid 序后面，禁止标题段插到最前再按文件塌缩（#1024 插队把正文段挤掉）；hybrid 已够 limit 个独特文件时不打标题第二趟。按文件塌缩后每个文件带 `passages`（同文件池内前 `PICO_KB_PASSAGES`=3 段，最高分段在前）：金标里文件已进 top5 但展示段不含答案的 17 例，12 例答案在该文件第 2/3 段；带 3 段后 quote@5 金标 0.451→0.627、held-out 0.809→0.851，file@5 不变（#1006 刀 3）。禁止按 `_matchesPosition` 数量挑代表段（金标掉、xlsx 掉）。New API `rerank-pro` 现网会打乱通用序，保持 `PICO_KB_RERANK=0`。整篇 JSON 按字段=值展开（与表格同一层）。`.md`/`.html` 入库要 `docling-slim[format-markdown,format-html]`（marko / bs4），缺包会 422。禁止 Pico 自选厂牌、禁止 PATCH 智谱/硅基 REST embedder。health 只写 `new-api`，不得写 zhipu/siliconflow。Meili 挂了失败 = 诚实 miss。禁每条 upsert `ensure()` 洪水 PATCH。清洪水用 `POST /tasks/cancel`。PDF 入库先认文字层；**文字层为空，或 ≥3 页且过半页 <80 字 → RapidOCR 渲页兜底进库（业主 2026-09-14 · #994 阶段 2 · #997）**，标 `ocr` / `sparse-layer` / `ocr-truncated`（`PICO_KB_OCR_MAX_PAGES` 默认 40），OCR 更短则留原层；认不出仍诚实 `empty`。png/jpg 走同一 OCR 口。OCR 模型用 `rapidocr` 轮子自带 ONNX，不下载、不装 torch。**kb/ingest 这条路 OCR 进库**；索引用提取全文（`markdown`），预览 slices 8×800 不得截断入库。扫描件正文要落同任务 `kb_text`，reindex（ocr=False）用这份，禁止靠 `/tmp` 脚本每次部后重灌。生产 `PICO_ENV=production` 时 `school-a` / `handtest-kb` / `regress-school` 等实验室租户不进索引；reindex-all 一律跳过。回形针→模型：原件 `input_file` 优先，提供商读不了才随行 OCR 文本；仍禁 Pico 自建 PDF 阅读器（#865 阅读器层）。旧版 `.doc/.xls/.ppt/.wps/.et/.dps` 与回形针同一条沙箱 soffice→OOXML，再走 Docling；转失败 422。Office 入库直接调 Docling 各格式后端（msword/msexcel/mspowerpoint/md/html），**禁止 `DocumentConverter`**——它导入即拉 `docling_parse`/torch，no-torch 镜像里必挂（现网 2026-09-14 前 Office 入库 100% 断）。xlsx/csv 索引用列名=格子（不是只逗号拼接）；markdown 表同样展开；切片按行切，一行不拆到两块。格式矩阵与 code 表：`docs/KB-INGEST-FORMATS.md`。联网检索不直连 DeepSeek official web_search：有钥才检索，无钥 = 诚实未检索。#733/#737/#950/#954。**联网渠道（#973 · 业主 2026-09-09）：** 一次部署只认一条上游，`PICO_SEARCH_PROVIDER=tavily`（缺省 · `TAVILY_API_KEY` · 免费 1000/月）或 `zhipu`（`ZHIPU_API_KEY` · search_std ¥0.01/次 · 业主说开才开）；各自价签 `pico:web_search` / `pico:web_search/zhipu`，usage extra 带 `channel_id`。禁止按问题类型分流、禁止跨渠道兜底、禁止 SearXNG/抓页进产品（ECS 实测：百度首查即验证码、搜狗 5 次崩、Bing 空页）。Brave / Google / OpenAI 直连从广东 ECS 不可达。
...
31. **加载诚实（#748）。** 办公天花板 `sandbox_office_lib` 常驻 CORE。`inspect_document` / `verify_document` 不常驻。`generate_*` / `sandbox_pptx_lib` 已不在网关；挂 Skill 不得再把它们当写路。焊句禁回潮。编程沙箱仍 EXTENDED。禁词表自动挂。只改 Python 说明 ≠ Pi 看见（仍认 §14）。
...
37. **聊天回形针 = 老师唯一上传口。** 图本轮进 Pi `images[]`；文档落 `POST /v1/files`。进本轮 user **只挂文件名 + 账本 id**（#865）。PDF/docx/xlsx/pptx **原件**直送 GPT Responses `input_file`（#876），Pico 不抽文、不 OCR、不渲页。**记住原件的账本 `run_id` 必须传给 true_pi**（#878）；对不上会直打 New API，模型只能读未读 excerpt。llm-pass 只认接到 127.0.0.1 套接字的连接，禁止只看 `client.host`（本机 eth0 / 转发头会是 172.x，Pi 会被 403）。按标题 `workspace_read_file` 跳过 `edu_excerpt`。旧 `.doc/.ppt/.xls` ingest **和送模型前** 走沙箱 soffice 转 OOXML；老师原名可仍是 `.doc`。转换失败不装绿：禁止写「已交给模型文件口」，禁止焊「另存为」。soffice 临时文件用 ASCII 名。预览打转换件。作曲栏粘贴：必须先 preventDefault 再读 files（Chromium 在 PD 前 types/files 都可能空）；有文件就回形针、禁止插文件名；无文件才还原 text/plain。落地页 `#pico-wb-home-input` 与 ChatForm 同一套；document 捕获阶段拦。模型仍可调 `workspace_read_file`。Pi 无文件口 ≠ 自研 PDF 核。`/c/new` 回形针先绑 `pending_*` 草稿 id，首句 rebind 到真会话；空会话残留才用 3 分钟/90 秒窗兜底。LibreChat `file`/`input_file` 文件名要进本轮（禁止静默丢掉后让模型说没收到）。LibreChat 本地仓不是老师柜。ingest HTTP 失败不得假装已传。「我的文件」是生成物柜。禁目的地三选 / VectorStore / 转到学校当上传。
...
48. **办公脚本在 `pico-office` 容器跑，不在 pico-api 里（#959）。** `sandbox_office_lib` 合同不变，后端 = 同 `pico-sandbox` 镜像的第二个 service：`network_mode: none`、`read_only`、tmpfs、uid 65532、unix socket `/run/pico-office/office.sock`，pico-api 用 `PICO_OFFICE_URL=unix://…` 进去；测试用 `embedded`（conftest），生产禁 `embedded`。箱内是完整 Python（stdlib / csv / pandas / Pillow / matplotlib / soffice），**Pico 不再维护 import 白名单 / AST jail / pathlib 桩 / 猴补 save**（`sandbox_exec.py` 已删）；「不要 bash」= 不给宿主 shell、不开宿主 Pi builtins、不注册 `exec` 类动词。收集规则：`OUTPUT_PATH` → 脚本记录的保存路径 → 工作目录最新 `*.kind`；都没有 = `sandbox.no_output` 人话。脚本报错原样回 stderr 让模型自己修。`artifact_id` 原件放宽到任何类型（CSV/TXT/PNG…），只有同 kind 的办公名才必须是真 OOXML。`sandbox_workspace_exec`（parse-only 假执行）已从网关拆除。加 rlimit 只在容器（`PICO_OFFICE_RLIMIT_NPROC`），共享宿主上 NPROC 帽会误杀。**部署合同（#964）：** socket 目录禁止命名卷（dockerd 建成 root:755，65532 bind 失败）。宿主绑定；`prod-update.sh` 只做 fetch/checkout，然后 `exec` 该 SHA 的 `prod-update.impl.sh`，改部署逻辑的当次部署就跑新脚本。impl 在 compose up 之前建 1777 sticky（宿主无 CAP_CHOWN，不假装 chown 65532）；不可写且非空失败闭合；孤儿命名卷必须删掉不能复用。绑定源换 inode 后 pico-api 必须 recreate 才能看见 socket。部署门必须探箱（`exit 11`），只探 pico-api health 会假绿。**在飞任务（#969 · 业主 2026-09-09）：** 老师 run 只活在 pico-api 进程里，recreate 就死；impl 在 build 后、recreate 前轮询 `/health` `inflight_runs`，有就等（`PICO_DEPLOY_DRAIN_WAIT_S` 默认 300），到点带 WARN 仍重建。业主允许打断；禁止 drain 模式 / 跨进程恢复 / 任务 OS。长任务尽量夜里部。真模型回归：`scripts/office-regress.py`（生产 loopback · 代理钥 + `X-Pico-Membership-Id` 合成租户 · 按 `kind` 找产物，同名替换是产品规则别按文件名找），部后跑、结果贴 Issue、不进 CI。
49. **办公三个数基线（#959 · 2026-09-09 · gpt-5.6-sol medium）。** T1a 公式 + T1b 续改：4 次工具 / 107s / 2 份；T2 CSV→xlsx+docx：8 次 / 234s / 2 份；T3 HTML+PPT：5 次 / 269s / 2 份；T4 Stop：cancelled · 0 产物。对照 #919 旧基线 T1a 类 10 次 / 97s / 7 份。墙钟主要是模型思考，不是工具往返。
50. **edu 侧栏 = 同一颗 Pico，只看这个人这一页（#975 · 正本 edu-core#604/#829）。** 认「附属，不是用户要求」标记；非 json_only 一律进真 Pi、CORE 同套手、不看 `model` 字串；SYSTEM 不带 day-use 柜；Pi 会话目录 `school/membership/conversation`（conversation id 由 edu 按老师+页发，Pico 不重放 `messages[]`）。请求带 `affordances[]` 才多一只 `propose_page_mutation`：只验 id、只记 staged、回顶层 `pico_mutations` + 一行 ChangeProposal，学校确认后自己执行；没能力表就如实说这页没这只手。提示词分两段：json_only 拟议段说 affordance（fill_cells/insert_col），Pi 全手段说工作台工具，互不描述对方没有的手。左页 web 表现在无窗口手（`inspect_document` 已下架），读不齐要明说。合同：`contracts/sidebar-session.md` · `tools.md §4.7`。
45. **模型与计量只走 New API。** 聊天/出图只打 `127.0.0.1:3000`（槽位可仍叫 `DEEPSEEK_*`）。渠道、密钥、轮询在 New API 管。Pico 只记 `usage_events`→积分，不做钱。edu 钱包只拉 export `points`，禁止再乘，禁止另接厂牌直连当第二账。本仓不写 edu。禁止为 DeepSeek/Kimi/Claude 再造直连核。知识库只挂载 Meili，禁止 Pico embedding 厂牌路由。#942 只证办公库调用面，不证老师聊天已走沙箱。
