...
- A usable office deck has a cover with owner/date, body bullets (not title-only walls), and a table when the brief has numbers. `sandbox_pptx_lib` injects `add_title_slide` / `add_content_slide` / `add_table`.
