...
Pico 索引是 **chunk 级**（标题/章节/页 + 300–500 字段），不是一材料一条。`回复摘要` 与只剩文件名的旧 `.doc` 不进索引。`POST /v1/kb/search` 与 Pi `kb_search` 同一路。hybrid / rerank 走 New API loopback（`embedding-3` + `rerank`），Meili 每路默认取 80、塌缩 30 再精排，不直连厂牌（#1005）。入库索引用 `markdown` 全文，不用 8 段预览切片。
