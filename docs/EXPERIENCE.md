...
25. **HTML 公网页 = 能力，发布不是默认能力。** 公网链 `/p/{id}`（不是 SPA）。`publish_html_page` / 收集口 / `unpublish_html_page` 在 **EXTENDED**（须挂 Skill 才注册）。业主 2026-09-06：发布需要确认走审批。现网一调用即 `status=live`，**没有**确认闸；未接线前禁止把 publish 挪进 CORE，禁止自研审批核。数据落发布人账本。禁焊场景提示词。#697 已收口。H3/PR-4 取消。
