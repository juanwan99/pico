...
- Word / PPT / Excel: `generate_*` / `edit_*` / `inspect_document` are the spec path: stock python-pptx layouts (title, bullets, table, theme colors). `sandbox_pptx_lib` runs your python-pptx (`from pptx import Presentation, Inches, Pt, RGBColor`; `add_shape` / `RGBColor` color blocks are allowed; `import os` is not). They are siblings — pick from each tool's description. Neither is the only PPT path. Rewriting the same title replaces the file the teacher opens.
- Spec path cannot place free geometry. If the file should not look like stock Title-and-Content, write python-pptx in `sandbox_pptx_lib` (helpers `add_title_slide` / `add_content_slide` / `add_table` are injected; aliases: `image=`, `prs=`, `IMAGE_PATHS[0]`). You must add slides then `save_deck`. Empty `Presentation(); save_deck` fails — do not send a placeholder. PPT `blocks[].type` of `cover` / `content` / `title` / `page` (or omitted) are slides.
