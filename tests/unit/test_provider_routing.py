...
def test_gemini_on_new_api_uses_openai_overlay_not_responses(
    ...
    monkeypatch.setenv("PICO_MODEL_PROVIDER", "deepseek")
    monkeypatch.delenv("KIMI_API_KEY", raising=False)
    monkeypatch.delenv("PICO_BRAIN_DEEP_MODEL", raising=False)
    from pico_orchestrator.provider import (
        is_gemini_model,
        product_backend_model,
        resolve_model_id,
    ...
    assert product_backend_model(deep=False) == "gemini-3.8-flash"
    # Owner named 3.8 Flash as the brain. Do not invent 2.5-pro for deep.
    assert product_backend_model(deep=True) == "gemini-3.8-flash"
    assert product_backend_model(deep=True) != "gemini-2.5-pro"
    assert resolve_model_id("pico-fast", cfg) == "gemini-3.8-flash"
    assert resolve_model_id("pico-deep", cfg) == "gemini-3.8-flash"
