...
_SINGLE_UNIT = re.compile(
    ...
    # P3: the 的 particle is common in natural Chinese (「可下载的 HTML 页面」);
    # without it, single-page requests fall through to structure enumeration
    # and get mis-minned (有件假失败).
    r"(?:做|生成|写|交付|准备|制作|给我|输出)\s*(?:可下载\s*(?:的\s*)?)?"
