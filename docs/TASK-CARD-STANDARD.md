...
派给执行窗时：派发条贴合同 Issue（`## 派发`），总管**当场** `spawn-executor --issue N`（SSH ecs grok）。只贴不 spawn = 没派。活窗禁再 spawn。无 ssh-ecs 拒领。禁止 `@cursor` 当执行者。模板 [`docs/templates/dispatch-slip.md`](./templates/dispatch-slip.md)。卡上无 `## 派发` = 没派。
