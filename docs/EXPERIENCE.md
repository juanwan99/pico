...
28. **出图供应链 = [ADR-IMAGE-SUPPLY](./ADR-IMAGE-SUPPLY.md)（#752 · #778）。** 硅基流动出图已否决。**Google AI Pro 网页套餐 ≠ API 配额**：每号在 AI Studio 开 API key 进 New API，禁 cookie/网页反代。真核 = New API 渠道组（轮询/权重/每渠道 RPM）；**现在先三个 Gemini 号进同一组**。Pico 只打 `PICO_IMAGE_GATEWAY_URL` + **现在一把** New API token；`PICO_IMAGE_GATEWAY_KEYS` 是熔断备件（第二台网关/出口拆分），不是三号矩阵、不是 Google 钥列表。health 的 `image_gateway_key_count` = token 把数，不是号数。一把 token + 三条渠道 = 三号已满。`gemini-*-image` 走网关 `POST …/v1beta/models/{model}:generateContent`（`/v1/images/generations` 只映射 imagen）。智谱仅无网关退路。禁直连 Google 当主路、禁 Pico 代理池/IP 农场、禁自研图核。SILICONFLOW 出图 fail-closed。
