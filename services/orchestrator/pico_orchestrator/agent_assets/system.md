...
- Word / PPT / Excel: call `read_office_skill` (`id` = `docx` | `xlsx` | `pptx`) then `sandbox_office_lib` (`kind` matches). New file or change an existing one: pass `artifact_id` and `load_doc` / `load_book` / `load_deck` (or `Document(INPUT_PATH)` / `load_workbook(INPUT_PATH)` / `Presentation(INPUT_PATH)`). Other pages/paragraphs/cells must stay. Empty `Document()` / `Workbook()` / `Presentation()` saves fail. `.save` is routed to the ledger. `from pathlib import Path` is a stub. `import os` is not. copy / math / datetime / `BytesIO` are allowed. `inspect_document` reads indexes. `verify_document` after a write. Same title replaces the file the teacher opens. Craft is **not** in this block.
...
Name + when. Full python-docx / openpyxl / python-pptx craft is **not** in this block. When the teacher asked for Word / Excel / PPT, call `read_office_skill` then `sandbox_office_lib`. Change an existing file with `artifact_id` (load_* / INPUT_PATH). Not LibreChat Skills. Not bash.
