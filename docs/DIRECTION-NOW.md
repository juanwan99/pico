# DIRECTION-NOW · 业主方向锁定（2026-08-09 · v1.4 北极星 2026-09-06）
...
SEE: HANDOFF-WB-PI · TRUTH-FREEZE v2.0 · STATE-NOW · #744 · #919
...
## 0-star. 北极星（2026-09-06 · BINDING · 继承 2026-08-24 用法 · v1.3 能力并列仍有效）
...
产品主线是办公：Word、Excel、HTML、PPT。写代码只够服务办公，不是编程产品。
能力并列，禁止焊死唯一路径。先扎实办公能力，再优化编排。
以后多源自动编排仍用上游 Pi，禁止自研第二编排核。
专用办公动词是捷径，不是天花板。
天花板 = 隔离面上用成熟办公库对真文件做事（对标 Claude/Codex 文档 skill：
  SKILL 工艺 + 真跑 python-docx / openpyxl / python-pptx + 检查），
  不是再加 generate_*，也不是 Word 里的 Copilot。
Skill 只能收窄白名单；不是权限裁剪器，也不能把已承诺能力藏到必须先挂 Skill。
工作环境（文件、程序执行、依赖、进程生命周期）交给成熟上游隔离环境。
本阶段接走的是办公计算机，不是通用 bash / 编程沙箱 / 自制 jail。
Pico 只做薄适配：身份授权、唯一账本、产品对象、交互与交付门闩。
新能力先问：哪段成熟方案接走职责、Pico 能否少维护行为。
加专用工具、加厚桥、加一个万能 exec 而旧定向协议照旧 = 不算进步。
验收看减法，不看演示多一个动词。
...
| **办公** | 产品要的：真 Word / Excel / HTML / PPT，打开能用、同聊能改 | 编程产品；对标 Codex/Cursor 写代码 |
| **并列** | 多项能力同时可被模型选用 | 焊死「必须真图 / 必须某厂 / 必须某工具」 |
...
- 把产品做成「仅教育课件生成器」  
- 把专用办公动词当能力上限；用 Skill 当权限裁剪器藏已承诺能力  
- 新增万能执行工具而旧定向协议与补偿逻辑全部照旧  
- 把「必须真图 / 必须某厂 / 必须某工具」写进目标句  
- 把产品做成编程 Agent；用 host bash / Pi 内建 bash / 自制 jail 当办公能力  
- 再加一张「加厚 spec」办公卡当进步  
...
产品主线 = 办公（Word / Excel / HTML / PPT）· 写代码是仆人
能力并列 · 禁焊死路径 · 专用动词是捷径 · 天花板 = 隔离办公库
工作环境交成熟上游 · 本阶段接办公计算机 · 不要 bash
Pico 只薄适配账本/授权/门脸 · 编排仍是上游 Pi
办事程度对标 WorkBuddy（六条）· 不是用启发式逼交件
不做：编程产品 · 连接器 · MCP · Skill 摊子 · 定向猜任务 · 自研第二核
CLAIM-WB: NO
```

```
════════════════════════════════════
BINDING · DIRECTION-NOW · v1.4 · 2026-09-06
Pico 用法 = Grok 用法 · 办公主线 · 能力并列 · 办公计算机交成熟上游
════════════════════════════════════
```
