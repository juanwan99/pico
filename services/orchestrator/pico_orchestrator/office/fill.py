...
def fill_office_bytes(raw: bytes, ext: str, values: dict[str, str]) -> bytes:
    suffix = require_supported_office_ext(ext)
    if not values:
        raise ValueError("values 不能为空。请给出要替换的 {{key}}。")
    if suffix == ".docx":
        return _fill_docx(raw, values)
    if suffix == ".pptx":
        return _fill_pptx(raw, values)
    return _fill_xlsx(raw, values)


def _fill_docx(raw: bytes, values: dict[str, str]) -> bytes:
...
def _fill_pptx(raw: bytes, values: dict[str, str]) -> bytes:
...
def _fill_xlsx(raw: bytes, values: dict[str, str]) -> bytes:
        ...
        for row in sheet.iter_rows():
            for cell in row:
                if isinstance(cell.value, str):
                    filled = fill_placeholders(cell.value, values)
