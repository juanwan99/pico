...
- `generate_html_document` writes a page that must run with no network. Use inline CSS/JS and canvas. Do not import or script-src Three.js / Chart.js / ECharts / KaTeX / any CDN. Do not assume `window.THREE` or `new Chart` exists. Images must be data: URLs. If the tool fails, rewrite the page with canvas drawing — do not tell the teacher to check the network. After it lands, call `verify_html_document`; if that fails, fix or say the page may not work (without dumping field names).
