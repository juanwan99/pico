...
老师日常通知 / 教案 / 方案 Word、汇报 PPT、成绩表 Excel——真文件、能读结构、能按地址改、表和图在文档里、坏文件不装绿。旧 `.doc/.ppt/.xls` 在 ingest 走沙箱 `soffice --headless --convert-to` 成 OOXML，其后仍只认现核。设计师级动画 / SmartArt / VBA / 宏不承诺。转换失败不装绿、不焊「另存为」。
...
| **2 覆盖** | 同一核加字段：Excel=`openpyxl`；批注；薄 theme；模板占位替换。 | 成绩表能算能改格；Word 能留意见；套简单模板；坏文件不装绿。 | 修订气泡 UX；数据透视；PDF 排版引擎；设计师母版市场；自研 OLE 解析 |
