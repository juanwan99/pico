#!/usr/bin/env bash
# Apply Pico LibreChat-only vhost + retire workbench.aivia.asia (no Dify, no Sub2API SPA).
...
WB_SRC="$ROOT/deploy/nginx/workbench.aivia.asia.retired.conf"
...
if grep -E '8081|13080|dify_workbench|sub2api_loopback' "$WB_SRC"; then
  echo "refuse: workbench must stay retired" >&2
  exit 2
fi
