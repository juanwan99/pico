...
_POLICIES: dict[str, SkillPolicy] = {
    "skill-deliverable": SkillPolicy(
        ...
        instruction=(
            ...
            "出图必须 generate_image（默认 cheap 无字示意图；要更细或照片感才 high）；"
            "失败用人话说明，禁止编造图，禁止用 SVG 冒充照片；中文说明写在 HTML 或 generate_diagram，不要让 cheap 档在像素上写字；"
            "图要进 Word/PPT 时把 artifact id 写入 spec；"
    ...
    "skill-engineering-delivery": SkillPolicy(
        ...
        instruction=(
            ...
            "禁止 generate_* 另造冒充改原件；出图用 generate_image（cheap/high）；结构图用 generate_diagram；其它用 workspace_write_file；"
