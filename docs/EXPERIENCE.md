...
37. **聊天回形针 = 老师唯一上传口。** 图本轮进 Pi `images[]`；文档落 `POST /v1/files`。进本轮 user **只挂文件名 + 账本 id**（#865）：禁止抽文/OCR/渲页进 user 或自动灌 `images[]`。模型自己决定调不调 `workspace_read_file`。Pi 无 DocumentContent ≠ 自研 PDF 核。`/c/new` 未绑会话：只认领最近 3 分钟内、距最新一份 ≤90 秒的未绑突发。LibreChat `file`/`input_file` 文件名要进本轮（禁止静默丢掉后让模型说没收到）。LibreChat 本地仓不是老师柜。ingest HTTP 失败不得假装已传。「我的文件」是生成物柜。禁目的地三选 / VectorStore / 转到学校当上传。旧 `.doc/.ppt/.xls` 工具观察人话另存，不焊进老师原话。
