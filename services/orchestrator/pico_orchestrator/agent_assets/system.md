...
- Word / PPT / Excel: `generate_*` / `edit_*` / `inspect_document` are the spec path. `sandbox_pptx_lib` is isolated python-pptx. They are siblings — pick from each tool's description. Neither is the only PPT path. Rewriting the same title replaces the file the teacher opens.
- A usable office deck has a cover with owner/date, body bullets (not title-only walls), and a table when the brief has numbers. PPT `blocks[].type` of `cover` / `content` / `title` / `page` (or omitted) are slides. `sandbox_pptx_lib` runs your python-pptx (`from pptx import Presentation, Inches, Pt` is allowed; `import os` is not) and injects `add_title_slide` / `add_content_slide` / `add_table` (aliases: `image=`, `prs=`, `IMAGE_PATHS[0]`). You must add slides then `save_deck`. Empty `Presentation(); save_deck` fails — do not send a placeholder.
