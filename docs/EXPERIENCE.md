...
37. **聊天回形针 = 老师唯一上传口。** 图本轮进 Pi `images[]`；文档落 `POST /v1/files`。进本轮 user **只挂文件名 + 账本 id**（#865）。PDF/docx/xlsx/pptx **原件**直送 GPT Responses `input_file`（#876），Pico 不抽文、不 OCR、不渲页。**记住原件的账本 `run_id` 必须传给 true_pi**（#878）；对不上会直打 New API，模型只能读未读 excerpt。llm-pass 只认接到 127.0.0.1 套接字的连接，禁止只看 `client.host`（本机 eth0 / 转发头会是 172.x，Pi 会被 403）。按标题 `workspace_read_file` 跳过 `edu_excerpt`。旧 `.doc/.ppt/.xls` ingest 走沙箱 soffice 转 OOXML，本轮把转换件交给模型文件口；老师原名可仍是 `.doc`。转换失败不装绿、不焊「另存为」，模型自己决定怎么说。作曲栏粘贴有 `clipboard.files` 必须 `preventDefault`，禁止只贴文件名。模型仍可调 `workspace_read_file`。Pi 无文件口 ≠ 自研 PDF 核。`/c/new` 回形针先绑 `pending_*` 草稿 id，首句 rebind 到真会话；空会话残留才用 3 分钟/90 秒窗兜底。LibreChat `file`/`input_file` 文件名要进本轮（禁止静默丢掉后让模型说没收到）。LibreChat 本地仓不是老师柜。ingest HTTP 失败不得假装已传。「我的文件」是生成物柜。禁目的地三选 / VectorStore / 转到学校当上传。
