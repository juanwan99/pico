...
- A usable office deck has a cover with owner/date, body bullets (not title-only walls), and a table when the brief has numbers. PPT `blocks[].type` of `cover` / `content` / `title` / `page` (or omitted) are slides. `sandbox_pptx_lib` runs your python-pptx (`from pptx import Presentation` is allowed; `import os` is not) and injects `add_title_slide` / `add_content_slide` / `add_table`. You must add slides then `save_deck`. Empty `Presentation(); save_deck` fails — do not send a placeholder.
