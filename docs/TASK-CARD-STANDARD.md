# 任务卡格式（指针 · 不再贴 315）

```text
DOC: docs/TASK-CARD-STANDARD.md
STATUS: BINDING v2.2 — 2026-08-24
FROZEN 卡面 — 禁止改本文件形状、复活 315
EXEC 三刀 BINDING:
  证据贴 Issue 评论 · 过门=公网结果句 · DONE=curl tip=main（或写明不部） · 本窗对账后关
SUPERSEDES: 四行短卡当派发形态
真源: docs/ONEFLOW.md v2.1 · 现况: docs/STATE-NOW.md
模板: .github/ISSUE_TEMPLATE/ · docs/templates/card-build.md · docs/templates/dispatch-slip.md
经验: docs/MEMORY-RESET.md（不进卡面；派发条只点名坑）
北极星: docs/DIRECTION-NOW.md §0-star v1.3 · 用法 = Grok · 能力并列 · 工作环境交成熟上游
```

点 **New Issue** 选「执行卡」或贴 **标准任务卡**（#627 体例）。卡必须自含【已锁事实】【IN】【验收】——跨窗零记忆，调查必须写进 Issue。

派给本窗时：派发条可贴合同 Issue（`## 派发`）。本窗合一：开卡/改/合/部/收尾同一窗。写码 `/home/ops/pico`；生产 `/opt/pico` 只 prod-update。禁止 spawn-executor / Cursor 云 Task（入口已断）。合前 `pr-ci-ready` 一眼，禁止窗里轮询 CI。模板 [`docs/templates/dispatch-slip.md`](./templates/dispatch-slip.md)。卡别拆太细。

## 卡面（Issue 合同）

标准任务卡体例（对齐 #627），骨架仍是四行：

```text
结果：老师在 pico.aivia.asia 能看见什么
不准：最多 5 条（必含：部前关卡 · 直推 main · ship-bff-web）
过门：最多 4 条人路径
部署：PICO_DEPLOY_SHA=<40位> bash /opt/pico/scripts/prod-update.sh
       没差写「不部」
```

禁止用只有这四行的短卡替代整张标准任务卡。

开窗戳（卡顶两问 + stamp-ok，无则拒领）：

```text
同域在飞？ 无 / 本卡续 / #<n>
残债新卡？ 否
```

回执五句，禁超 15 行：

```text
CLAIM T-ID · 本窗 · DONE
PR：#
SHA：live = origin/main =
过门：T1 过/不过 · 证据
剩下：无
PASS：未签 / 本窗对账已关
收尾：三行已刷 · 本卡已关（白名单除外） · 同域 PR 已清 · 卫生勾完
```

## 从 315 留下的（纪律，不填表）

这些是旧卡真正有用的部分，写进门禁，不要再贴 S/G/E：

| 留下 | 怎么守 |
|------|--------|
| 一张结果一张卡 | 「结果」只能一句老师能看见的 |
| 残债同卡续 | 禁 `T-*-DEBT`；不过就在本卡评论续 |
| 改 + 测 + 部同一张 | 禁「码已合、本卡只部」另开 |
| GitHub 是合同 | Issue / PR / SHA；**派发条只做入口**；聊天不算状态 |
| 开窗两问才能领 | 无 stamp-ok 拒领 |
| 调查进 Issue | 已锁事实写进合同；派发条点名必读 + ≤3 坑 |
| 生产认 40 位 SHA | `curl -fsS https://pico.aivia.asia/api/pico/tip` |
| 合了没装 = 没做完 | live tip 对不上 origin/main 不算过（docs-only 写明不部） |
| 卡过门本窗关 | live tip 该对齐已对齐且结果句可见 → 本窗对账关（白名单 #316 #449 #170 #634 #475 除外才留开）。CLAIM-WB / 全球 PASS 仍不代签 |
| 同域一张在飞 | 两问第一句 |
| 视觉默认关 | 过门用 API/SHA；非业主点名不开浏览器 |

**故意不贴回卡面：** 工具 ID 清单、经验全文、S4 缓冲 30 分钟、F/T 大表。  
工具自己翻手册。本周坑在 [`MEMORY-RESET.md`](./MEMORY-RESET.md)。挡这张卡的坑，写进「不准」最多一条。

## 不要做

- 把本文件旧长表、`DAY-TASK-*.md`、已关 Issue 当现行格式
- 把 edu 的 315 卡面抄进 pico
- 经验写进 Issue 模板默认值
- 用四行短卡当已派
- 不贴派发条、或把 Issue 全文当派发
- 把调查留在聊天
- **定向卡**：读正文猜任务、force_agent 自动挂交付、把「必须交文件」焊进 user prompt（北极星 DIRECTION-NOW §0-star）
- 问题没讨论清就开执行 PR（调查先写 Issue；无 stamp-ok 禁开工）
