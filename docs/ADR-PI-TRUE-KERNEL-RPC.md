...
| **工具** | `--no-builtin-tools` + 仓内 extension 仅注册 Pico gateway **7** 工具白名单；经 127.0.0.1 回调 Python |
...
4. 桥保持薄：进程 + JSONL + 7 工具回调 + 事件映射；不做第二 OS。
...
**允许：** spawn RPC · JSONL prompt/abort · 事件映射 · 7 工具回调 · session 目录 · 显式 shadow diff  
