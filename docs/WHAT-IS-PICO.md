# Pico 到底是什么（2026-08-01 正本清源）

```
DOC: docs/WHAT-IS-PICO.md
STATUS: BINDING · 覆盖一切冲突的产品口述与旧文档金句
OWNER: 业主目标 + 总管落盘
TRUTH: 本页「是/不是/现状/目标」；代码与 DEPLOYED/TEST REPORT 可更新「现状」；不可偷偷改「目标」
```

---

## 0. 一句话

**Pico = 面向学校场景的「独立 AI 工作台」产品底座：**  
人在浏览器里下任务 → 模型与工具干活 → **过程与产物记在 Pico 唯一 AI 账本** → 需要改业务时走 **人确认（S7）** → 业务数据仍归 **edu**（后置对接）。

它不是网盘，不是教务系统，不是「再造一个大模型」，也不是「已经接好开源 Kimi Agent 运行时」的成品叙事。

---

## 1. 是什么（产品）

| 维度 | 定义 |
|------|------|
| **品类** | 类 Claude / Codex / WorkBuddy 的 **任务型 AI 工作台**（对话 + 办事 + 产物） |
| **用户** | 教师/管理者等（学校场景），先独立可试用 |
| **壳** | **`apps/librechat`（MIT）** 中文工作台；禁止回潮 web/nextchat/workbench；禁止拆闭源 WorkBuddy |
| **智能** | **云端模型 HTTPS API**（**Kimi / Moonshot 优先**，可 DeepSeek 等） |
| **过程真源** | **Pico 唯一 AI 账本**：Task / Run / Event / Artifact / Change(S7)… |
| **控制面** | 租户/membership、工具白名单、停止、重试、技能策略、限流与安全门 |
| **与 edu** | **Pico = AI 过程真源**；**edu = 业务数据真源**；真联调后置；**禁止写 edu-cloud** |

**用户可感知的成功：**  
打开公网工作台 → 登录 → 下任务 → 看到过程 → 拿到产物 → 能停、能找回、失败能再试 → 状态不撒谎。

---

## 2. 不是什么

| 不是 | 说明 |
|------|------|
| 网盘 / 5GB 文件主产品 | 文件是产物与附件，不是产品中心 |
| 教务 SaaS / 成绩主库 | 学籍班课考在 edu |
| 自托管大模型训练集群 | 默认买 API |
| 自研「Agent OS」终局品牌 | **目标禁止**；代码里若有薄工具环 = **待归位债务** |
| 「开源 Kimi Agent 已深度接入」 | **当前不成立**（见 §4）；禁止再当完成态宣传 |
| Live Preview 沙箱端口故事 | 业主主路径是 **公网 HTTPS** |

---

## 3. 结构（四层）

```text
1. 壳     LibreChat 工作台 UI
2. 控制面  Pico API：身份、Run 生命周期、产物、S7、安全
3. 编排核  目标 vs 现状见 §4
4. 模型    Kimi 等 HTTPS API
```

---

## 4. 编排 · 目标 vs 现状（彻底诚实）

### 4.1 目标（BINDING · 业主原意）

```text
编排 = 开源 Kimi Agent（钉版本）薄改接入
     + Pico 账本 / 白名单 / S7 / 停止·重试控制面
模型 = Kimi HTTPS API（锁定）
禁止：自研 Agent OS 当产品主叙事或终局
禁止：预写「Plan B 换 Pi/其它」进架构真源；走不通再议
```

### 4.2 现状（2026-08-01 代码事实）

```text
模型：Kimi HTTPS API                          ✅ 在用
多步：AsyncOpenAI + run_agent_loop 自研薄环    ⚠️ 过渡实现
kimi-agent-sdk / kimi-cli：pin + 读 yaml 安全证明  ⚠️ 非主执行器
「已用开源 Kimi Agent 编排」                    ❌ 名实不符
```

偏航锚点：约 2026-07-29 多步环进仓 → 2026-07-30 PR #30 合 main。  
文档校准曾重申目标，但未强制改核。

### 4.3 即日起纪律

1. 禁止宣称编排已是开源 Kimi Agent。  
2. 归位完成前禁止往自研环堆「小 OS」能力。  
3. 允许短期保留环作 TRANSITIONAL 执行器，必须在本页/债表标明。  
4. 归位完成 = 生产多步 Run 主路径由开源 **Kimi Agent** 运行时驱动，事件入 Pico 账本。  
5. 刷新/历史/停止/重试属控制面与通路，不得用来证明编排目标已达成。  
6. **编排路径唯一：开源 Kimi Agent（+ Kimi 模型）。**  
   - **禁止**在真源/架构文档中预埋 Plan B、Pi、OpenCode 或其它运行时备选（以免执行窗误会可自行换核）。  
   - 若 Kimi Agent **证伪走不通**：停止换核尝试，**书面交业主重新讨论**；不得擅自切换运行时。

---

## 5. 阶段（摘要）

| 阶段 | 状态 |
|------|------|
| 公网可跑 / 主链可演示 | 大体具备 |
| 日用可靠 | 进行中 |
| **编排归位** | **未完成 · 路线 P0** |
| edu / 像素终局 | 后置 |

---

## 6. HARD

只写 `juanwan99/pico`；禁 edu-cloud；禁 PROXY=1；禁打印密钥；GitHub 为进度真源。

---

## 7. 三句记忆

1. Pico 是 AI 工作台底座（壳 + 账本 + 控制面 + 模型 API），不是网盘/教务。  
2. Kimi 模型在用；开源 Kimi Agent 编排是目标，不是已完成现状。  
3. 自研工具环是过渡债务，禁止假称薄改已完成。
